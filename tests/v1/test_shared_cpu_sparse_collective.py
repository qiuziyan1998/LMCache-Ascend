...
class _PoisonSparseConsumer:
    def batched_to_gpu_head_token_wise(self, **_kwargs):
        raise AssertionError("metadata-only retrieve created an NPU consumer")
...
def test_sparse_passive_metadata_only_extends_without_npu_consumer(monkeypatch):
    ...
    engine = object.__new__(AscendLMCacheEngine)
    engine.num_layers = 1
    engine.gpu_connector = _PoisonSparseConsumer()
    engine.token_database = _FakeTokenDatabase(key)
    engine.shared_cpu_cache_passive_allocator = _FakePassiveAllocator(mem_obj)
    engine.shared_cpu_cache_generation = 7
    engine.metadata = type("Meta", (), {"first_rank": 0})()
...
def test_sparse_rank0_metadata_only_publishes_without_npu_consumer(monkeypatch):
    monkeypatch.setattr(
        ascend_cache_engine,
        "assert_layerwise_gpu_connector",
        lambda _connector: None,
...
def test_sparse_rank0_metadata_only_rejects_incomplete_pointer_preflight(
    ...
    next(retriever)
    with pytest.raises(ValueError, match="incomplete source pointer coverage"):
        retriever.send((None, 0))
