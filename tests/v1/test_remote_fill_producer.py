...
@pytest.mark.parametrize(
    "change", ["none", "owner", "storage", "event", "range", "native_failure"]
)
@pytest.mark.parametrize("separate_jobs", [False, True])
def test_prefix_readiness_reuse_is_job_local_and_validates_every_range(
    change, separate_jobs
):
    import torch

    h = _PrefixRepairHarness()
    payload = b"".join(h.payload[p.canonical_key] for p in h.pages)
    owners = [torch.tensor(list(payload), dtype=torch.uint8)]
    owner_refs = [weakref.ref(owners[0])]
    events = [SimpleNamespace(synchronize=Mock())]
    plans = 0
    ...
    def source(missing):
        ...
        if plans:
            if change == "owner":
                owners[0] = owners[0].clone()
                owner_refs.append(weakref.ref(owners[0]))
            elif change == "storage":
                owners[0].resize_(len(payload) + 64)
            elif change == "event":
                events.append(SimpleNamespace(synchronize=Mock()))
        plans += 1
        page = missing[0]
        offset = 0 if page is h.pages[0] else h.pages[0].expected_bytes
        ptr = owners[0].data_ptr() + offset
        if plans > 1 and change == "range":
            ptr = owners[0].data_ptr() + owners[0].numel()
        return DirectPushSourcePlan(
            (DirectPushPageSource(page.canonical_key, page.kv_group,
                                  (ptr,), (page.expected_bytes,)),),
            tuple(owners), (events[-1],),
        )

    def prepare(plan):
        # Mirror the real connector's validation and readiness, with host owners.
        coordinator_module.validate_direct_push_source_ranges(
            tuple(ptr for page in plan.pages for ptr in page.source_ptrs),
            tuple(size for page in plan.pages for size in page.source_lengths),
            coordinator_module.direct_push_owner_ranges(plan.owners),
        )
        for event in plan.producer_events:
            event.synchronize()
        return h.prepare(plan)

    def submit(**kwargs):
        if change == "native_failure" and h.transferred:
            future = Future()
            future.set_exception(NativeDirectPushTerminalError(
                NativeDirectPushResult(h.attempt, -1, 0, 0, 0.0)
            ))
            return future
        return h.submit(**kwargs)
    ...
    coordinator = RemoteFillCoordinator(
        config=config, tp_size=8,
        fatal_reporter=RemoteFillProducerMetrics().snapshot,
        storage_manager=SimpleNamespace(prepare_remote_fill_source=prepare,
                                        submit_remote_fill_direct_push=submit),
    )
    state = ProducerRequestState(handoff=h.handoff, session=h.session)
    ...
    try:
        results = []
        for pages in ((h.pages[:1], h.pages[1:]) if separate_jobs else (h.pages,)):
            coordinator.submit_probe("prefix", state, pages, 6, maximum=1,
                                     context=None, source_factory=source)
            results.extend(state.last_future.result(timeout=3))
        assert results[0].direct_satisfied
        failed = change in ("range", "native_failure")
        assert results[1].direct_satisfied is (not failed)
        assert h.finish().direct_satisfied is (not failed)
        assert state.queued_windows == state.queued_bytes == 0
        if not failed:
            assert h.cached == h.payload
            expected_prepares = 1 if change == "none" and not separate_jobs else 2
            assert h.events.count("prepare_source") == expected_prepares
            assert sum(
                event.synchronize.call_count for event in events
            ) == expected_prepares
        else:
            assert len(h.transferred) == 1
            assert h.events.count("ArmWindowRequest") == (
                1 if change == "range" else 2
            )
        coordinator.close(())
        owners.clear()
