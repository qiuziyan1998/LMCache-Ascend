...
class AscendLMCacheEngine(LMCacheEngine):
    ...
    def _append_shared_handle_cache(
        ...
        *,
        ...
        if len(layer_handles) < chunk_offset:
            raise ValueError(
                "Shared handle cache does not cover the requested tail offset: "
                f"layer_id={layer_id}, cached={len(layer_handles)}, "
                f"chunk_offset={chunk_offset}"
            )
        del layer_handles[chunk_offset:]
    ...
    def _retrieve_layer_head_token_wise_shared_passive(
        ...
        **kwargs,
        ...
        passive_total_started = time.perf_counter()
        assert self.gpu_connector is not None, (
        ...
        request_configs = kwargs.get("request_configs")
        kv_group = kwargs.get("kv_group", 0)
        refresh_prefix_chunks = int(
            kwargs.get("shared_cpu_refresh_prefix_chunks", 0) or 0
        )
        ...
        assert_layerwise_gpu_connector(self.gpu_connector)
        consumer_init_started = time.perf_counter()
        mem_obj_consumer = self.gpu_connector.batched_to_gpu_head_token_wise(
            **kwargs
        )
        next(mem_obj_consumer)
        consumer_init_ms = (time.perf_counter() - consumer_init_started) * 1000
        ...
        def append_passive_chunk_metadata() -> None:
            ...
            expected_total = refresh_prefix_chunks + expected_handle_count
            if cached_starts is not None:
                if len(cached_starts) < refresh_prefix_chunks:
                    raise ValueError(
                        "Sparse shared CPU passive metadata does not cover tail "
                        f"offset: req_id={req_id}, kv_group={kv_group}, "
                        f"cached={len(cached_starts)}, "
                        f"offset={refresh_prefix_chunks}"
                    )
                del cached_starts[refresh_prefix_chunks:]
                cached_starts.extend(starts[refresh_prefix_chunks:expected_total])
            if cached_ends is not None:
                del cached_ends[refresh_prefix_chunks:]
                cached_ends.extend(ends[refresh_prefix_chunks:expected_total])
            if cached_keys is not None:
                if not cached_keys:
                    cached_keys.extend([] for _ in range(self.num_layers))
                for cache_layer_id, layer_keys in enumerate(keys_layer_major):
                    del cached_keys[cache_layer_id][refresh_prefix_chunks:]
                    cached_keys[cache_layer_id].extend(
                        layer_keys[refresh_prefix_chunks:expected_total]
                    )
        ...
        try:
            for layer_id in range(self.num_layers):
                ...
                record_passive_phase("envelope_wait", layer_id, phase_started)
                phase_started = time.perf_counter()
                ...
                record_passive_phase("envelope_validate", layer_id, phase_started)
                if envelope.status in ("miss", "skipped"):
                    ...
                    mem_objs_layer: list[MemoryObj] = []
                    phase_started = time.perf_counter()
                    for local_chunk_index, handle in enumerate(envelope.handles):
                        chunk_index = refresh_prefix_chunks + local_chunk_index
                    ...
                    record_passive_phase("view_create", layer_id, phase_started)
                    phase_started = time.perf_counter()
                    try:
                        ...
                        for mem_obj in mem_objs_layer:
                            if mem_obj.is_valid():
                                mem_obj.ref_count_down()
                        raise
                    record_passive_phase("cache_append", layer_id, phase_started)
                    to_release.extend(mem_objs_layer)
                    self._append_shared_handle_cache(
                        layer_id,
                        envelope.handles,
                        cached_shared_handles,
                        chunk_offset=refresh_prefix_chunks,
                    )
                ...
                record_passive_phase("consumer_send", layer_id, phase_started)
                elapsed_layer_ms = (time.perf_counter() - layer_started) * 1000
                layer_total_ms += elapsed_layer_ms
                if elapsed_layer_ms > max_layer_total_ms:
                    max_layer_total_ms = elapsed_layer_ms
            ...
            logger.info(
                "[BOOTSTRAP_ASCEND_SHARED_PASSIVE_SUMMARY] "
                "req=%s kv_group=%s layers=%d chunks_per_layer=%d "
                "metadata_ms=%.3f consumer_init_ms=%.3f "
                "layer_total_ms=%.3f max_layer_total_ms=%.3f "
                "max_layer_total_id=%d envelope_wait_ms=%.3f "
                "envelope_wait_max_ms=%.3f envelope_wait_max_layer=%d "
                "envelope_validate_ms=%.3f view_create_ms=%.3f "
                ...
                kv_group,
                self.num_layers,
                expected_handle_count or 0,
                metadata_ms,
                consumer_init_ms,
                layer_total_ms,
                max_layer_total_ms,
                max_layer_total_id,
                phase_totals_ms["envelope_wait"],
                phase_max_ms["envelope_wait"],
                phase_max_layer["envelope_wait"],
                phase_totals_ms["envelope_validate"],
                phase_totals_ms["view_create"],
                phase_max_ms["view_create"],
                phase_max_layer["view_create"],
                phase_totals_ms["cache_append"],
                phase_totals_ms["consumer_send"],
    ...
    def _retrieve_layer_head_token_wise_bootstrap(
        ...
        **kwargs,
        ...
        if (
            ...
            and (not has_shared_cached_retrieve or refresh_prefix_chunks > 0)
        ):
        ...
        tail_refresh = refresh_prefix_chunks > 0
        if tail_refresh:
            if refresh_prefix_chunks >= required_chunks:
                raise ValueError(
                    "Shared CPU tail refresh offset must precede the new tail: "
                    f"req_id={kwargs.get('req_id', 'unspecified')}, "
                    f"kv_group={kv_group}, offset={refresh_prefix_chunks}, "
                    f"required_chunks={required_chunks}"
                )
            cached_prefix_counts = [
                self._min_layer_cache_chunks(cache, self.num_layers)
                for cache in (cached_tensors, cached_memory_objs)
                if self._has_retrieve_data_cache(cache, None, self.num_layers)
            ]
            if not cached_prefix_counts or any(
                count != refresh_prefix_chunks for count in cached_prefix_counts
            ):
                raise ValueError(
                    "Shared CPU tail refresh cache does not match its stable "
                    f"prefix: req_id={kwargs.get('req_id', 'unspecified')}, "
                    f"kv_group={kv_group}, cached={cached_prefix_counts}, "
                    f"offset={refresh_prefix_chunks}"
                )

        if (
            has_cached_retrieve_data
            and required_chunks
        ...
        if (cached_tensors_cover or tail_refresh) and cached_tensors is not None:
        ...
        materialize_mode = "layerwise"
        preflight_retrieve_keys = (
            [layer_keys[refresh_prefix_chunks:] for layer_keys in retrieve_keys]
            if tail_refresh
            else retrieve_keys
        )
        preflight_starts = starts[refresh_prefix_chunks:] if tail_refresh else starts
        preflight_ends = ends[refresh_prefix_chunks:] if tail_refresh else ends
        if (
            shared_sparse_retrieve
            and not use_cached_retrieve
            and preflight_retrieve_keys
        ):
            ...
            if sampled_worker_retrieve:
                ...
                try:
                    for layer_keys in preflight_retrieve_keys:
                ...
                for layer_id, layer_keys in enumerate(preflight_retrieve_keys):
                    ...
                    shared_chunk_locations_layer_major.append(layer_locations)
            location_scan_ms = (time.perf_counter() - phase_started) * 1000
            if missing_locations:
                ...
                try:
                    ...
                    capacity_details = self._shared_cpu_runtime_capacity_details(
                        ...
                        kv_group=kv_group,
                        keys_layer_major=preflight_retrieve_keys,
                ...
                if not capacity_details.get("fits", False):
                    ...
                    try:
                        ...
                        if remote_layers_per_batch > 1 and remote_only:
                            ...
                            for layer_id, layer_keys in enumerate(
                                preflight_retrieve_keys
                            ):
                                layer_materialize_started = time.perf_counter()
                                if sampled_worker_retrieve:
                                ...
                                layer_materialize_ms = (
                                    time.perf_counter()
                                    - layer_materialize_started
                                ) * 1000
                                if layer_materialize_ms > materialize_max_ms:
                                    materialize_max_ms = layer_materialize_ms
                                    materialize_max_layer = layer_id
        ...
        if shared_sparse_retrieve and not shared_retrieve_passive:
            local_chunks = sum(
                location_name == LOCAL_CPU_BACKEND_NAME
            ...
            logger.info(
                "[BOOTSTRAP_ASCEND_SHARED_PREFLIGHT_SUMMARY] "
                "req=%s kv_group=%s layers=%d chunks_per_layer=%d "
                "use_cached=%s sampled_worker=%s local_chunk_refs=%d "
                "remote_chunk_refs=%d metadata_ms=%.3f location_scan_ms=%.3f "
        ...
        layer_phase_totals_ms = {
            ...
            "store_fence": 0.0,
            "publication_claim": 0.0,
            "handle_build": 0.0,
            "envelope_broadcast": 0.0,
            "consumer_send": 0.0,
        }
        ...
        def record_rank0_layer_phase(
            phase_name: str,
            layer_id: int,
            started: float,
        ) -> None:
            elapsed_ms = (time.perf_counter() - started) * 1000
            layer_phase_totals_ms[phase_name] += elapsed_ms
        ...
        def ensure_mem_obj_consumer():
            ...
            next(mem_obj_consumer)
            consumer_init_ms += (
                time.perf_counter() - consumer_init_started
            ) * 1000
            return mem_obj_consumer
        ...
        try:
            for layer_id in range(self.num_layers):
                ...
                if publish_shared_handles:
                    ...
                    try:
                        if not shared_store_publication_fenced:
                            ...
                            record_rank0_layer_phase(
                                "store_fence", layer_id, phase_started
                            )
                ...
                consumer = ensure_mem_obj_consumer()
                phase_started = time.perf_counter()
                if sparse_payload is not None:
                    sparse_payload["memory_objs_layer"] = mem_objs_layer
                    consumer.send(sparse_payload)
                else:
                    consumer.send(
                        (
                            mem_objs_layer,
                ...
                record_rank0_layer_phase(
                    "consumer_send", layer_id, phase_started
                )
                elapsed_layer_ms = (time.perf_counter() - layer_started) * 1000
                rank0_layer_total_ms += elapsed_layer_ms
                if elapsed_layer_ms > rank0_max_layer_total_ms:
                    rank0_max_layer_total_ms = elapsed_layer_ms
                    rank0_max_layer_total_id = layer_id
            ...
            if shared_sparse_retrieve and not shared_retrieve_passive:
                logger.info(
                    "[BOOTSTRAP_ASCEND_SHARED_RANK0_LAYER_SUMMARY] "
                    "req=%s kv_group=%s layers=%d chunks_per_layer=%d "
                    ...
                    layer_phase_max_ms["handle_build"],
                    layer_phase_max_id["handle_build"],
                    layer_phase_totals_ms["envelope_broadcast"],
                    layer_phase_max_ms["envelope_broadcast"],
                    layer_phase_max_id["envelope_broadcast"],
