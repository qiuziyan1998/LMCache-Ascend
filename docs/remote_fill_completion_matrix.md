...
|---|---|---|---|
| V1 connector forwarding | `vllm-ascend/tests/ut/kv_connector/test_dsa_index_connector.py` covers placement, metrics, fatal forwarding, and conflicting-owner rejection | `PENDING_HARDWARE` | Construct the actual configured AscendMultiConnector with its real LMCache and Mooncake children |
...
| V4 operation deadlines and replay | Every operation timeout and exact lost-reply replay are covered by `LMCache-NPU/tests/v1/remote_fill/test_transport.py` and `test_state.py` | `PASS_UNIT` | None before H0; real armed-DMA loss remains prohibited |
...
| H0 A--G | `remote_fill_h0.py` production-adapter runner and strict validator | Hardware report not run |
| C1 nine-scenario matrix | `remote_fill_c1.py` fixed production-adapter runner | TP8/DP2/MTP report not run |
| P1 A/B/C workload | client runner, comparator, trace builder, fixed campaign runner, strict aggregate evaluator | Performance campaign not run |
